...
import { LeaderboardViewEntry } from '../interfaces/interfaces';
...
export class AddMatch {
    ...
    public SelectedPlayers: LeaderboardViewEntry[];
    public Players: Player[];

    public Player1: LeaderboardViewEntry;
    public Player2: LeaderboardViewEntry;
    public Player3: LeaderboardViewEntry;
    public Player4: LeaderboardViewEntry;
    ...
    addPlayer(entry: LeaderboardViewEntry) {
        if (this.SelectedPlayers.includes(entry)) {
            console.log("player already in list, removing!");

            var index = this.SelectedPlayers.indexOf(entry);
            this.SelectedPlayers.splice(index, 1);

        } else {
            if (this.SelectedPlayers.length < 4) {
                console.log("adding player " + entry.UserName);
                this.SelectedPlayers.push(entry);
            }
            else {
        ...
        this.SelectedPlayers.sort((e1, e2) => e1.EloRating - e2.EloRating);
        this.Player1 = this.SelectedPlayers[0];
        this.Player2 = this.SelectedPlayers[1];
        this.Player3 = this.SelectedPlayers[2];
        this.Player4 = this.SelectedPlayers[3];
    }
    ...
    addResult() {
        ...
        if (this.Match1.MatchResult.Team1Score != null) {
            ...
            // this.http.fetch('http://staging-foosball9000api.sovs.net/api/leaderboard/index', {
            //     method: 'post',
            //     body: json(this.Match1)
            // })
            //     .then(result => result.json() as Promise<LeaderboardView[]>)
            //     .then(data => {
            //         this.leaderboards = data;
    ...
    GetGravatar(email: string, size: number) {
